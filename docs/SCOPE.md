...
| General `Socket/Bind/Connect/Listen/Accept4` for arbitrary (not `AF_UNIX`-to-a-known-host-service) networking | **Borderline — needs a named use case per addition** | Winsock already reaches ordinary TCP/UDP through Wine adequately; test 1 (speed) hasn't been demonstrated for this case, and test 3 doesn't apply once it's not a host socket Winsock literally can't reach. Kept for now because it's small and shared code with `DialUnix`, not because it's independently justified — flagged so it doesn't silently grow (e.g. TLS, HTTP, or protocol-specific helpers on top of it would not pass the test and should not be added here). |
| `Mmap`/`Munmap` | **Borderline — no cited use case yet** | No current `f4` call site and no named UX improvement in any doc. Kept because it's cheap and generically useful for a future large-file-viewer use case, but this is exactly the "POSIX for completeness" pattern the scope test exists to catch. **Action: either cite a concrete use case (e.g. a viewer/editor that would use it) in this file, or remove it before the next release.** |
